...
module.exports = class LineProcessor {
  ...
  connect(tokens, callback) {
    ...
    const args = autocomplete.split(tokens, ' ', 2);
    let [scheme, authority] = autocomplete.split(args[0], '://', 1, true);
    if (authority === undefined) {
      authority = scheme;
      scheme = DEFAULT_SCHEME;
    }
    const [host, portString] = autocomplete.split(authority, ':', 2, true);
    let port;
    if (!host) {
      callback(reportMissingArgument('Host'));
      return;
    ...
    const interfaces = args[2] ? autocomplete.split(args[2], ' ') : [];
