...
class CallCompleter {
  ...
  complete(inputs, depth) {
    ...
    let completions = autocomplete.complete(iface, Object.keys(this.cli.api));
    if (method === undefined || !completions.some(el => el === iface)) {
      return [completions, depth];
    }

    completions = autocomplete.complete(method, this.cli.api[iface]);
...
module.exports = class CommandProcessor extends EventEmitter {
  ...
  complete(inputs, depth) {
    ...
    return [autocomplete.complete(cmd, completions), depth + 1];
  }
