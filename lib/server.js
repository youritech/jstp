...
const SimpleSessionStorageProvider = require(
...
const initServer = function(
  applications, authPolicy, sessionStorageProvider, heartbeatInterval, listener
) {
  ({
  ...
  this._clientsBySessionId = new Map();

  this.sessionsStorage = sessionStorageProvider ||
                         new SimpleSessionStorageProvider();
  ...
  this.on('handshakeRequest', (connection, appName, authStrategy) => {
    this.emit(
      'log', connection, 'handshakeRequest', appName, authStrategy
    );
  });
...
class Server {
  ...
  startSession(connection, application, strategy, credentials, callback) {
    const sessionStorageProvider = application.sessionsStorage ||
    ...
    this.authenticate(
      ...
      (error, username) => {
        ...
        this._createSession(
          username, connection, sessionStorageProvider, callback
        );
      }
    );
  }
  ...
  restoreSession(connection, application, credentials, callback) {
    ...
    const getSessionCallback = (session) => {
      if (!session) {
        callback(errors.ERR_AUTH_FAILED);
        return;
      }
    ...
    if (existingConnection) {
      getSessionCallback(existingConnection.session);
    } else if (sessionStorageProvider.isAsync()) {
      sessionStorageProvider.get(sessionId, getSessionCallback);
    } else {
      getSessionCallback(sessionStorageProvider.get(sessionId));
    }
  }
  ...
  _onClientDisconnect(connection) {
    ...
    if (connection.session) {
      this._clientsBySessionId.set(connection.session.id, null);
    }
  }
}
