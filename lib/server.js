...
const initServer = function(
  applications, authPolicy, sessionStorageProvider, heartbeatInterval,
  clientExpirationTime, listener
) {
  ({
...
class Server {
  ...
  restoreSession(connection, application, credentials, callback) {
    ...
    if (existingConnection) {
      const replacementTransport = connection.transport;
      Object.keys(connection._transportListeners).forEach((event) => {
        connection.transport.removeListener(
          event, connection._transportListeners[event]
        );
      });
    ...
    if (sessionStorageProvider.isAsync()) {
      sessionStorageProvider.get(sessionId, null, session =>
        getSessionCallback(session, callback)
      );
    } else {
