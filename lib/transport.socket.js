...
sock.createServer = (config, applications, authCallback) => {
  ...
  const transportServer = new SocketServer(rawServer, config, secure);
...
//   secure - whether TLS is used
//
function SocketServer(server, config, secure) {
  ...
  const connectEventName = secure ? 'secureConnection' : 'connection';
