...
//
ws.createServer = function(config, applications,
                           authCallback, originCheckStrategy) {
  var secure = false;
  ...
  var jstpServer = new Server(wsServer, applications, authCallback,
    config.heartbeat);
...
//
ws.createClient = function(config, application) {
  if (typeof(config) === 'string') {
    config = { url: config };
  }

  var wsClient = new JstpWebSocketClient(config);
  var jstpClient = new Client(wsClient, application, config.heartbeat);
