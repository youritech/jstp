...
function createDefaultReconnector() {
  ...
  const randomMsecGen = () =>
    Math.floor((RANDOM_OFFSET_MAX + 1) * Math.random());
...
//
const newConnectFn = (
  connFactory, transportClass, transportName
) => (app, client, ...options) => {
...
const newReconnectFn = (
  ...
  connFactory(...options, (error, rawConnection) => {
    ...
    connection.client.connectPolicy(
      connection._serverApp, connection, connection.session,
      (error, connection) => {
        if (error) {
          callback(error, connection);
          return;
