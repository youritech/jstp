...
const newConnectFn = (
  ...
  const callback = common.unsafeCallback(options);
...
const newConnectAndInspectFn = (connFactory, transportClass) => {
  ...
  return (app, client, interfaces, ...options) => {
    const callback = common.safeCallback(options);
    connect(app, client, ...options, (error, connection) => {
      if (error) {
        callback(error);
        return;
      }
      const proxiesCollector = metasync.collect(interfaces.length);
      interfaces.forEach((name) => {
        connection.inspectInterface(name, (error, proxy) => {
          proxiesCollector.collect(name, error, proxy);
        });
      });
      proxiesCollector.done((error, proxies) => {
        callback(error, connection, proxies);
      });
    });
  };
};
