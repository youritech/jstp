...
Connection.prototype._processHandshakeRequest = function(packet, keys) {
  ...
  var callback = (
    username ?
    this._onSessionCreated.bind(this, true) :
    this._onSessionCreated.bind(this, false)
  );
...
//   isAuthenticated - true for authenticated sessions, false for anonymous
//   error - error that has occured or null
//   sessionId - session id or hash
//
Connection.prototype._onSessionCreated =
  function(isAuthenticated, error, sessionId) {
    if (error) {
