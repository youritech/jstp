...
Connection.prototype.callback = function(packetId, error, result) {
  var packet = error ?
    this.createPacket('callback', null, 'error', error) :
    this.createPacket('callback', null, 'ok', result);
...
Connection.prototype.inspect = function(interfaceName, callback) {
  ...
  this.callbacks[packetId] = function(error) {
    ...
    var clientEmit = proxy.emit;

    proxy.emit = function(eventName, eventArgs, dontRetranslate) {
      if (!dontRetranslate) {
        self.event(interfaceName, eventName, eventArgs);
      }
    ...
    for (var i = 0; i < methods.length; i++) {
      self.wrapRemoteMethod(proxy, interfaceName, methods[i]);
    }
...
// method
//   proxy - remote proxy object
//   interfaceName - name of an interface
//   method - name of a method
//
Connection.prototype.wrapRemoteMethod =
  function(proxy, interfaceName, methodName) {
    var self = this;
    ...
    proxy[methodName] = function() {
      ...
      self.call(interfaceName, methodName, args, callback);
    };
  };
